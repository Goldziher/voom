...
```text
roots ──▶ scan ──▶ classify ──▶ policy ──▶ delete ──▶ report
          (0005)    (0002)      (0004)     (0006)     (0007)
```
...
4. **delete** — canonicalize, check the protected denylist and containment, refuse symlinks
   and filesystem crossings, then `remove_dir_all`. Failures are per-artifact and reported.
5. **report** — sort deterministically by path, render human or JSON from one result set.
