...
async function installBinary() {
  try {
    ...
    const archivePath = path.join(binDir, isZip ? `${BINARY}.zip` : `${BINARY}.tar.gz`);
    const binaryName = os.type() === "Windows_NT" ? `${BINARY}.exe` : BINARY;
    ...
    console.log(`Downloading ${BINARY} binary from ${url}...`);
    ...
    console.error(`Error installing ${BINARY} binary:`, error.message);
    process.exit(1);
  }
}
